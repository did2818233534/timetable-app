...
桌面组件：`TimetableRepository.timetable.value` → 可见星期/节次过滤 → `BuildRollingSchedule` → 动态 `RemoteViews` 行列 → Android Launcher。主表格与组件共用 `SelectSlotDisplays`，并由 `splitDisplaySlots` 决定每格显示一门还是上下分栏两门；课程按“本周有效、最近未来、最近过去”的顺序选择。
