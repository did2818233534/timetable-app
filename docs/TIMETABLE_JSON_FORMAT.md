...
- `splitDisplaySlots`：需要上下分栏同时显示两门课程的格子；未列出的格子只显示一门。
...
`splitDisplaySlots` 按星期和节次配置需要分栏的格子。例如让周二第 2 节显示上下两门课程：
...
未配置的格子只显示本周课程；如果本周没有课，则灰色显示最近将要上的课程，未来也没有时显示最近上过的课程。分栏格最多显示两门，超过两门会显示剩余数量，点击格子仍可查看和编辑全部课程。旧 JSON 省略此字段时默认为空数组。
...
- 导入会先完整解析并校验，成功后新增一份课表并自动切换；已有课表不会被覆盖。
- 无效日期、时间、节次、周数和单双周值会拒绝导入。时间冲突会保留并弹出警告，由用户决定如何处理。
- 同一星期和节次可以包含多门课程；应用按“本周正在上、最近未来、最近过去”的顺序选择最多两门，分别显示在上、下半格。超过两门时仍可点开格子管理全部课程。
- 文件最大为 1 MB。
- 导入后的数据保存在应用私有课表库中，不会写回所选文件。
