...
面向 Android 和小米澎湃 OS 的本地优先课程表应用。主界面和桌面小组件均采用表格布局，支持单双周、起止周、教室、自定义节次与上下课时间、文件导入和应用内编辑。
...
- JSON 中的 `periods` 完整决定每天节数、上下课时间与课间分隔，不依赖学校固定作息。
- 支持左右滑动切换周次；点按课程可编辑备注和周数，点按空白格可新增，长按课程与空白格可复制粘贴。
